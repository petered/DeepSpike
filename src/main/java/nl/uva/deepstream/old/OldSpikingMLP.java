...
interface EventConsumer<EventType>{
...
class OldSpikingMLP extends BaseMLP {
    ...
    public OldSpikingMLP(double[][][] ws, int n_steps, double eta, boolean depth_first,
        ...
        List<double[][]> weights = Arrays.asList(ws);
        ...
        for (int i=0; i<n_layers; i++)
            layers[i] = new QuantizingLayer(ws[i], eta, fractional_updates, n_steps,
                    forward_discretize, backward_discretize, test_discretize, rng, i==0, regularization);
        int input_size = weights.get(0).length;
        int[] layer_sizes = weights.stream().mapToInt(w->w[0].length).toArray();
    ...
    public double[] predict_one(double[] input_vec) {
        ...
        double[] out_sum = output_summer.get_sum();
        double[] output = new double[out_sum.length];
        for (int i=0; i<out_sum.length; i++)
            output[i] = out_sum[i] / ((double) n_steps);
        return output;
    }
    ...
    public void train_one(double[] input_vec, double[] target_vec){
        ...
        for (int t = 0; t < n_steps; t++) {
            ...
            training_router.handle_event(new VectorEvent(neg_target_vec), neg_target_quantizer::feed_event);
            if (hold_error)
...
class QuantizingLayer{
    ...
    public QuantizingLayer(double[][] w, double eta, boolean fractional_updates,
...
class WeightModule extends StatelessBaseEventModule<VectorEvent>{
    ...
    public WeightModule(double[][] w) {
        this.w = w;
        this.n_in = w.length;
        this.n_out = w[0].length;
    }
...
interface PassThroughQuantizer extends Quantizer{
    ...
    public void feed_spike(SignedSpikeEvent spike);  // Shortcut method
...
abstract class BaseQueueHerder extends BaseHerder<SignedSpikeEvent> implements PassThroughQuantizer{
    ...
    protected void flush_queue(){
        while (!queue.isEmpty()){
//            System.out.println("Sending Event from unit "+queue.peek().ev.src + " of layer of size "+phi.length);
...
class RectVarQueueHerder extends BaseQueueHerder implements Quantizer{
    ...
    public void create_spikes_from_unit(int unit_ix) {
        while(phi[unit_ix] > 0.5){
            queue.add(new DispatchEvent(new SpikeEvent(unit_ix), phi[unit_ix]));
            phi[unit_ix] -= 1;
        }
    }
...
class ScaledRoundingHerder extends BaseQueueHerder{
    ...
    void create_spikes_from_unit(int unit_ix) {
        while(true){
            SignedSpikeEvent ev;
            if (phi[unit_ix] > sensitivity/2)
                ev = new SignedSpikeEvent(unit_ix, false);
            else if (phi[unit_ix] <= -sensitivity/2)
                ev = new SignedSpikeEvent(unit_ix, true);
            else
                break;
            int sign = ev.is_negative?-1:1;
            queue.add(new DispatchEvent(ev, Math.abs(phi[unit_ix])));
            phi[unit_ix] -= sign*sensitivity;
        }
    }
...
class PoissonHerder extends BaseQueueHerder{
    ...
    public void create_spikes_from_unit(int unit_ix) {
        ...
        while (true){
            SignedSpikeEvent ev;
            if (phi[unit_ix] > thresholds[unit_ix])
                ev = new SignedSpikeEvent(unit_ix, false);
            else if (phi[unit_ix] <= -thresholds[unit_ix])
                ev = new SignedSpikeEvent(unit_ix, true);
            else
                break;
            queue.add(new DispatchEvent(ev, Math.abs(phi[unit_ix])));
            if (zero_mode)
                ...
                phi[unit_ix] -= ev.get_sign();
            thresholds[unit_ix] = next_random_threshold();
        }
    }
...
class OuterProductModule extends BaseEventModule<MatrixEvent> implements WeightGradientModule<MatrixEvent>{
    ...
    public void produce_event(){
        ...
        send_event(new MatrixEvent(outer_prod));
    }
    ...
    @Override
    public void feed_error_event(WeightedSpikeEvent event) {
//        System.out.println("Weighted outptu eve ");
...
class VectorEvent extends BaseEvent{
    ...
    public VectorEvent(double[] vec) {
        this(vec, false);
    }

    public VectorEvent(double[] vec, boolean safe){
        // Set safe to true if you're certain that the array won't be modified later and you want to save time/memory.
        this.vec = safe ? Arrays.copyOf(vec, vec.length) : vec;
    }
...
class BooleanVectorEvent extends BaseEvent{
    ...
    final public boolean[] vec;

    public BooleanVectorEvent(boolean[] vec) {
        this(vec, false);
    }

    public BooleanVectorEvent(boolean[] vec, boolean safe){
        this.vec = safe ? Arrays.copyOf(vec, vec.length) : vec;
    }
}
...
class MatrixEvent extends BaseEvent{
    ...
    public MatrixEvent(double[][] matrix){
        this.matrix = matrix;
    }
}
...
class ColumnChangeEvent extends BaseEvent{
    ...
    ColumnChangeEvent(int row_ix, double[] vec){
        this(row_ix, vec, false);
    }

    ColumnChangeEvent(int row_ix, double[] vec, boolean safe){
        this.col_ix = row_ix;
        this.vec = safe ? Arrays.copyOf(vec, vec.length) : vec;
    }
...
//        ModuleLinks<EventType> links = src_deque.poll();
        ModuleLinks<EventType> links = src_deque.pollLast();
        if (links == null)
            return;
//        System.out.println("Clearing Queue from: "+links.src);
